    ...
    public static void main(String[] args){
        ...
        for (int j = 0; j < q; j++) {
            ...
            switch (command) {
                ...
                case "redo": {
                    if (!undo.isEmpty()) {
                        String redoOperation = undo.pop();
                        operations.push(redoOperation);
                        String[] redoParts = redoOperation.split(" ");
                        if (redoOperation.startsWith("insert")) {
                            int i = Integer.parseInt(redoParts[1]) - 1;
                            char x = redoParts[2].charAt(0);
                            history = history.substring(0, i) + x + history.substring(i);
                        } else if (redoOperation.startsWith("delete")) {
                            int i = Integer.parseInt(redoParts[1]) - 1;
                            history = history.substring(0, i) + history.substring(i + 1);
                        }
                    }
