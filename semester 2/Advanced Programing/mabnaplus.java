...
public class mabnaplus {

    public static void main(String[] args) {
        while (true){
            Scanner myObj = new Scanner(System.in);
            String input =  myObj.nextLine();String input2 =  myObj.nextLine();
            ArrayList<Character> number = new ArrayList<>(create_array(input));
            ArrayList<Character> number2 = new ArrayList<>(create_array(input2));

            int mabna1 = Integer.parseInt((myObj.nextLine()));
            int mabna2 = Integer.parseInt(myObj.nextLine());
            int dot = find_dot(number);int dot2 = find_dot(number);
            float decimal = to_decimal(number , dot , mabna1);
            float decimal2 = to_decimal(number2 , dot2 , mabna1);
            float amlgar = decimal+decimal2 ;

            for (Character ch : decimal_to(amlgar,mabna2)) {

                System.out.print(ch);
            }
            System.out.println();
            float amlgar2 = decimal-decimal2 ;
            for (Character ch2 : decimal_to(amlgar2,mabna2)) {
                System.out.print(ch2);
            }
            System.out.println();
            float amlgar3 = decimal*decimal2 ;
            for (Character ch2 : decimal_to(amlgar3,mabna2)) {
                System.out.print(ch2);
            }
            System.out.println();
            float amlgar4 = decimal/decimal2 ;
            for (Character ch2 : decimal_to(amlgar4,mabna2)) {
                System.out.print(ch2);
            }
